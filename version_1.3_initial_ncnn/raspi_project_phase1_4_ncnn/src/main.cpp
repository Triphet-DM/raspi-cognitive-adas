...
static AppConfig parse_args(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (key == "--model" || key == "--ncnn-param") cfg.model_path = value(key);
            ...
            std::cout
                << "Usage:\n"
                << "  ./app --ncnn-param ../src/models/best512s_ncnn.param "
                << "--ncnn-bin ../src/models/best512s_ncnn.bin --imgsz 512 --threads 3 "
...
int main(int argc, char** argv) {
    try {
        ...
        if (!cfg.hide_window) {
            cv::namedWindow("Phase 1.4 - NCNN", cv::WINDOW_NORMAL);
        }
        ...
        while (g_running) {
            ...
            if (!cfg.hide_window && !cfg.no_draw) {
                cv::imshow("Phase 1.4 - NCNN", canvas_bgr);
