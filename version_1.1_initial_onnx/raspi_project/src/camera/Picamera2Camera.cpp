...
class CppPicamera2:
    ...
    if (PyRun_SimpleString(py_code) != 0) {
        PyErr_Print();
        throw std::runtime_error("Failed to initialize Picamera2 Python bridge");
    }
    ...
    if (!camera_) {
        PyErr_Print();
        throw std::runtime_error("Failed to create Picamera2 camera object");
    }
}
...
bool Picamera2Camera::read(cv::Mat& frame) {
