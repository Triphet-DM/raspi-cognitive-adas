...
static AppConfig parse_args(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (key == "--model") cfg.model_path = value(key);
        ...
        else if (key == "--threads") cfg.onnx_threads = std::stoi(value(key));
            ...
            std::cout
                << "Usage:\n"
                << "  ./app --model ../src/models/best512s.onnx --imgsz 512 --threads 3 "
...
int main(int argc, char** argv) {
    try {
        ...
        std::cout << "===== Phase 1.3 Optimized =====\n";
        std::cout << "Pipeline     : Camera -> YOLO ONNX -> Draw/NoDraw\n";
        ...
        std::cout << "Threads      : " << cfg.onnx_threads << "\n";
        ...
        if (!cfg.hide_window) {
            cv::namedWindow("Phase 1.3 - Optimized", cv::WINDOW_NORMAL);
        }
        ...
        while (g_running) {
            ...
            if (!cfg.no_draw) {
                ...
                cv::cvtColor(frame_rgb, canvas_bgr, cv::COLOR_RGB2BGR);
                draw_phase1_overlay(
                    canvas_bgr,
                    detections,
                    times,
            ...
            if (!cfg.hide_window && !cfg.no_draw) {
                cv::imshow("Phase 1.3 - Optimized", canvas_bgr);
